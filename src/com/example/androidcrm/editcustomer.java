...
public class editcustomer extends Activity implements OnClickListener,
	...
	private static String SOAP_ACTION4 = "http://tempuri.org/getforeditcustomers";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		website.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				// TODO Auto-generated method stub
				if (!Patterns.WEB_URL.matcher(website.getText().toString())
						.matches()) {
					Toast.makeText(getApplicationContext(),
							"Enter a Valid Website", Toast.LENGTH_SHORT).show();
				}
			}
		});
		...
		emailcust.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				if (!checkEmail(emailcust.getText().toString())) {
					Toast.makeText(getApplicationContext(),
							"Enter a Valid Email Address", Toast.LENGTH_SHORT)
							.show();
				}
			}
		});
		...
		mobilenumber.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				if (s.length() != 10) {
					Toast.makeText(getApplicationContext(),
							"Enter a Valid Phonenumber", Toast.LENGTH_SHORT)
							.show();
				}
			}
		});
		AsyncCallWS5 task3 = new AsyncCallWS5();
		// Call execute
		task3.execute();
	...
	private void abc() {
		...
		String b = "not provided";
		if (custwebsite.equals(b)) {
			website.setText("");
		} else {
			website.setText(custwebsite);
		}
		mobilenumber.setText(a);
		emailcust.setText(custemail);

		System.out.println("adzfad:" + custtype);
		if (!custtype.equals(b)) {
			for (int i = 0; i < customertype.size(); i++) {

				if (custtype.equals(customertype.get(i))) {
					pos = i;
				}
			}
			customertypedisplay.setSelection(pos);
		} else {
			customertypedisplay.setSelection(0);
		}
		for (int i = 0; i < industryname.size(); i++) {
			if (custindustry.equals(industryname.get(i))) {
				pos = i;
			}
		}
	...
	public void onClick(View v) {
		...
		if (v.equals(save)) {
			...
			if (checkEmail(custemail) && a.length() == 10 && Patterns.WEB_URL.matcher(custwebsite)
					.matches()) {
				...
				// Call execute
				task.execute();
			}else if(!Patterns.WEB_URL.matcher(custwebsite)
						.matches()) 
			{
				...
				Toast.makeText(editcustomer.this, "Invalid Email Addresss",
						Toast.LENGTH_SHORT).show();
			...
			try {
				try {
					...
					myAndroidHttpTransport.call(SOAP_ACTION
							+ "storeupdatedcustomers", mySoapEnvelop);
				...
				SoapPrimitive response = (SoapPrimitive) mySoapEnvelop
						.getResponse();
		...
		protected void onPostExecute(Void result) {
			...
			if (!errored) {
				...
				if (loginStatus == 1) {
					...
					Toast.makeText(getApplicationContext(), "Updated customer",
							Toast.LENGTH_SHORT).show();
					Bundle b = new Bundle();
					b.putInt("a", 3);
					Intent i = new Intent(editcustomer.this, MainActivity.class);
	...
	private class AsyncCallWS5 extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			try {
				...
				tablerow = (SoapObject) table.getProperty(0);
				custname = tablerow.getProperty("AccountName").toString();

				custemail = tablerow.getProperty("Email").toString();

				custmobileno = Long.parseLong(tablerow.getProperty("Phone")
						.toString());
				a = tablerow.getProperty("Phone").toString();

				custwebsite = tablerow.getProperty("Website").toString();
				custindustry = tablerow.getProperty("IndustryName").toString();
				indusid = Long.parseLong(tablerow.getProperty("ID").toString());
				custtype = tablerow.getProperty("accounttype").toString();
				System.out.println("customertype:" + custtype);
				custypeiid = Long.parseLong(tablerow.getProperty(
